...
public class TradeExample {
  ...
  public static void testTrade() throws IOException {
    ...
    System.out.printf("get fee rates %s%n",((BybitAccountService)exchange.getAccountService()).getFeeRates(
        BybitCategory.LINEAR,null) );
    System.out.printf("get orders %s%n",exchange.getTradeService().getOrder("test_id123") );
    ...
    String marketFutureOrderId =
        exchange
            .getTradeService()
            .placeMarketOrder(
                new MarketOrder(OrderType.BID, new BigDecimal("0.05"), ETH_USDT_PERP));
    System.out.println("Market Future order id: " + marketFutureOrderId);

    //short
    LimitOrder limitOrderFuture = new LimitOrder(OrderType.ASK, new BigDecimal("0.05"), ETH_USDT_PERP,
